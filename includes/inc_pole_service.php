...
$viewdisplay_pm3 = $view->set_display('block_1');
...
if (!empty($view->result)) {
  ...
  $output = '<div id="service" class="bloc-pole"><h3 class="classe-h3">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_pm3).'</div>';
...
//     $outputEmpty = '<div id="MODIFIER"><h3 class="classe-h3">'.$view->get_title().'</h3>'.$emptyText.'</div>';
     $outputEmpty = '<div id="service" class="bloc-pole"><h3 class="classe-h3">'.$view->get_title().'</h3>'.$emptyTextVue.'</div>';
